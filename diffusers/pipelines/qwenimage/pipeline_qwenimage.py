...
def find_phrases_indices(prompt: str,
                         phrases: Union[str, List[str]],
                         tokenizer,
                         max_length: int = 50) -> List[Dict[str, Any]]:
    """
    Find token indices for given phrases in the prompt.
    phrases can be a single string or a list of strings.

    returns: List[Dict[str, Any]]
    [
        {"phrase": "fire truck", "range": (3, 4), "token_ids": [...], "text": "..."},
        {"phrase": "red", "range": (2, 2), "token_ids": [...], "text": "..."},
        ...
    ]
    """

    # make sure phrases is a list
    ...
    def find_subsequence(full_list, sub_list):
        '''Find all occurrences of sub_list in full_list and return their (start, end) indices.'''
        matches = []
    ...
    for phrase in phrases:
        ...
        ranges = find_subsequence(prompt_ids, tokens)
...
class QwenImagePipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def __call__(
        ...
        # which layers to skip/modulate the text conditions
        ...
        modulated_phases: Optional[Union[str,List[str]]]=None,
        ...
        if modulated_layers is not None and modulated_scales is None:
            modulated_scales = 2.0
            
        if modulated_phases is None:
            modulated_ranges=None
        else:
            # self.tokenizer_2    
            tmp_results = find_phrases_indices(
                prompt,
                modulated_phases,
                self.tokenizer,
                max_sequence_length
                )
            modulated_ranges = [x['range'] for x in tmp_results]
